...
// Lib
use Dpay\Logs\Database\Data\ParsedEventRecord as Record;
use Dpay\Abstracts\Database\MeekroDB\AbstractDatabaseTable;

/**
 * Handles Logging Parsed Webhook events to database
 * 
 * @method bool    insert(Record $r)  Insert Log Entry
 * @method Record  newRecord()                      Return instance of Record Data Class
 */
class ParsedEvents extends AbstractDatabaseTable {
	const TABLE  = 'app_parsed_events';
	const COLUMNS = [
		...
		'raw_apieventdata'    => ['LONGTEXT', ''],
        'raw_parsedeventdata' => ['LONGTEXT', ''],
	];

    const PRIMARYKEY = ['rid'];
	const RECORD_CLASS = '\\Dpay\\Logs\\Database\\Data\\ParsedEventRecord';
